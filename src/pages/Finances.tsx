...
import { DollarSign, CheckCircle, Clock, AlertTriangle, ShieldCheck, Calculator, ArrowRight } from "lucide-react";
...
export default function Finances() {
  ...
  const totalDebt = chargements.filter(c => c.statutPaiement === 'non_paye').reduce((sum, c) => sum + (c.solde || 0), 0);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex gap-4">
             <div className="bg-slate-50 dark:bg-slate-950 px-8 py-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                ...
                <p className="text-2xl font-black text-rose-500 tabular-nums">{totalDebt.toLocaleString()} XOF</p>
             </div>
             <div className="bg-slate-50 dark:bg-slate-950 px-8 py-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-1">En attente</p>
                <p className="text-2xl font-black text-amber-500 tabular-nums">{pendingCount}</p>
             </div>
          </div>
        </div>
      </div>
        ...
        {confirmPayId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white rounded-[2.5rem] p-10 max-w-lg w-full shadow-2xl border border-slate-100"
            >
              <div className="bg-emerald-100 w-20 h-20 rounded-3xl flex items-center justify-center text-emerald-600 mb-8">
                <ShieldCheck className="w-10 h-10" />
              </div>
              <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-4">Quittance Finale</h3>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  className="px-8 py-4 text-slate-400 font-bold uppercase hover:bg-slate-50 rounded-2xl transition-all"
                >
                  Annuler
                </button>
              </div>
...
function ChargementFinanceCard({ chargement, onPayRequest }: { chargement: any; onPayRequest: () => void; key?: string }) {
  ...
  const isOverdue = !isPaid && (new Date().getTime() - new Date(chargement.createdAt).getTime() > 7 * 24 * 60 * 60 * 1000);
    ...
    <div className={`overflow-hidden rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none transition-all duration-300 ${isPaid ? 'opacity-70' : 'hover:border-blue-500/50'} ${isOverdue ? 'ring-2 ring-rose-500 shadow-rose-500/20' : ''}`}>
      <div className="px-8 py-5 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center transition-colors">
         <div className="flex items-center gap-4">
            ...
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">
              Cas d'Usage — Dossier BL #{chargement.dossierId.slice(-4)} • Vecteur: {isInternal ? 'Flotte Interne' : 'Sous-traitance Prestataire'}
            </p>
         </div>
         {isOverdue && (
         ...
         <p className="text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-[0.2em]">
           Indexé le {new Date(chargement.createdAt).toLocaleDateString()}
         </p>
      </div>
      ...
      <div className="p-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-8">
          <FinanceItem 
            label="MATRICULE UNITÉ" 
            value={chargement.numeroConteneur || "NP-ALPHA"} 
            subtext="Identifiant EVP"
          />
          <FinanceItem 
            label="PROVISION UNITAIRE" 
            value={`${(chargement.avance || 0).toLocaleString()} F`} 
          ...
          <FinanceItem 
            ...
            value={`${(chargement.prixTotal || 0).toLocaleString()} F`} 
            subtext="Engagement Marché"
          />
          <FinanceItem 
            label="SOLDE NET PASSIF" 
            value={`${(chargement.solde || 0).toLocaleString()} F`} 
            ...
            {isPaid ? (
               ...
               >
                  ...
                  <span className="relative z-10 flex items-center gap-3">
                    <Clock className="w-4 h-4 text-amber-500 animate-spin-slow" /> Valider Quittance
                  </span>
               </button>
            )}
          </div>
...
function FinanceItem({ label, value, subtext, highlight = false, highlightColor = "text-white" }: { label: string, value: string | number, subtext: string, highlight?: boolean, highlightColor?: string }) {
  return (
    <div className="flex flex-col items-center text-center p-6 border border-transparent hover:border-slate-50 dark:hover:bg-slate-950/50 rounded-[2rem] transition-all group">
      ...
      <p className={`text-2xl sm:text-3xl font-black mb-2 tracking-tighter tabular-nums ${highlight ? highlightColor : "text-slate-900 dark:text-white"}`}>
        {value}
      </p>
